                                               ...
                                               bool random_colors*/)
{
  ...
  for (int row = 0; row < pcHeight; row++) {
    for (int col = 0; col < pcWidth; col++) {
      ...
      if(pcl_cloud->points[position].label == 4) counter++;
      cvCloud.push_back(p);
    }
  }
...
/*
  ...
  unsigned position = 0;
  unsigned centerRow = 62,row_begin = 89,row_end = 150;//35/89
  unsigned centerCol = 409,col_begin = 389,col_end = 442;//385/433
